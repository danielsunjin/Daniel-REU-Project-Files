...
load("/Users/daniel/Downloads/Archive/noreadcsf.rda")
load("/Users/daniel/Downloads/Archive/connectivity.rda")
load("/Users/daniel/Downloads/Archive/response.rda")
temp=connectivity[,,1]
...
#sum(riskfactors[,2]==3)
library(dplyr)
riskfactors=response[,2:dim(response)[2]] %>% select(Day0_Learning_Slope, Day1_Contextual_Freezing, Day2_Tone_Freezing)
...
png("/Users/daniel/Downloads/short2dnet2.png", units="in", width=7, height=7, res=400)  
...
for (i in subnetsresults) {
  regions <- append(regions, toString(i[2,], width = NULL))
  total_weight <- append(total_weight, i[7])
}

postertable <- data.frame(regions, total_weight)
write.csv(postertable, "/Users/daniel/Downloads/algo2.csv", row.names = FALSE)
...
Sex=response$Sex; Sex[Sex=="male"]="M"; Sex[Sex=="female"]="F"; 
Sexname=Sex; Sexname[Sexname=="M"]="Male" ;  Sexname[Sexname=="F"]="Female";
...
response_with_age_cat <- response %>%
  mutate(Age_Cat = case_when(
    Age_Months < 16 ~ "12",
    Age_Months >= 16 ~ "18"
  ))

Age=response_with_age_cat$Age_Cat
