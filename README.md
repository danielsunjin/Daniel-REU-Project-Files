...
The file [r_reader_connectome.R](https://github.com/danielsunjin/Daniel-REU-Project-Files/blob/main/Archive/r_reader_connectome.R) reads the connectome and behavior metrics data. Note that the variables storing the path to the master mouse datasheet, the path to the connectomes (the directory with the connectome CSV files that are named by Badea ID), and the path to the behavior.rda file may need to be updated. Here you can also specify which CSF (cerebral spinal fluid) regions in the connectome to not include in vertex screening or SCCA by listing the region indices of the DSURQE atlas plus 1 in the noreadcsf vector(for example, if we didnt want to include DSURQE atlas region 57, we would list region 58 in the noreadcsf vector). This script outputs important data as rda files (noreadcsf.rda, connectivity.rda, and response.rda).
